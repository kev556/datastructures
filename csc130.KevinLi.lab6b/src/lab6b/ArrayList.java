...
* <p>
* <b>Description:</b><br>
* ArrayList class. Defines an ArrayList, a data type capable of storing, searching, and 
* sorting objects. Contains sortAscending and sortDescending, sorting algorithms utilizing
...
public class ArrayList<T> implements List<T>, Comparable<List<T>> {
	 ...
	 */
	public ArrayList<T> sortAscending(java.util.Comparator<T> comparator){
		...
		for (int i = 0; i < dataArr.length - 1; i++) {
			for (int j = 0; j < (dataArr.length - i - 1); j++) {
				if (comparator.compare(dataArr[j], dataArr[j + 1]) > 0) {
					T temp = dataArr[j];
		...
		for (int i = 0; i < dataArr.length; i++) {
			newList.add(dataArr[i]);
		}
		// return newList
		return newList;
	}
	public ArrayList<T> sortDescending(java.util.Comparator<T> comparator){
		ArrayList<T> newList = new ArrayList<T>(size);
		T[] dataArr = toArray();
		// use a selection, insertion, or exchange sort to sort the data in dataArr
		// compare with comparator.compare(  )
